...
docker run --rm -v "$(pwd)"/drupal/custom-modules:/code dcycle/php-lint:3 --standard=DrupalPractice /code
docker run --rm -v "$(pwd)"/drupal/custom-modules:/code dcycle/php-lint:3 --standard=Drupal /code
